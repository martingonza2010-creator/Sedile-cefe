    ...
    if (form) {
        form.onsubmit = async (e) => {
            ...
            const data = {
                ...
                estatura_m: estatura,
                sexo,
                actividad, // Restored for V3.6 (Requires SQL update)
                tmt,
...
function initAssessmentLogic() {
    ...
    document.getElementById('btnOpenAssessment').onclick = () => {
        console.log("Opening Assessment View..."); // Debug Log
        const dashView = document.getElementById('view-dashboard');
        const assessView = document.getElementById('view-assessment');

        if (dashView && assessView) {
            ...
            console.error("Critical: Views not found in DOM");
        }
    };
